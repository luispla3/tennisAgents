...
def get_players() -> tuple[str, str]:
    ...
    player1 = questionary.text(
        "Introduce el nombre del Jugador 1:",
        validate=lambda x: len(x.strip()) > 0 or "Por favor, introduce un nombre válido.",
        style=questionary.Style([("text", "fg:green")]),
    ).ask()
    if not player1:
        print("\n[red]No se introdujo el Jugador 1. Saliendo...[/red]")
        exit(1)

    player2 = questionary.text(
        "Introduce el nombre del Jugador 2:",
        validate=lambda x: len(x.strip()) > 0 or "Por favor, introduce un nombre válido.",
        style=questionary.Style([("text", "fg:green")]),
    ).ask()
    if not player2:
        print("\n[red]No se introdujo el Jugador 2. Saliendo...[/red]")
        exit(1)
...
def get_tournament() -> str:
    ...
    tournament = questionary.text(
        "Introduce el nombre del torneo:",
        validate=lambda x: len(x.strip()) > 0 or "Por favor, introduce un nombre válido.",
        style=questionary.Style([("text", "fg:green")]),
    ).ask()
    if not tournament:
        print("\n[red]No se introdujo el torneo. Saliendo...[/red]")
        exit(1)
...
def select_shallow_thinking_agent(provider) -> str:
    ...
    SHALLOW_AGENT_OPTIONS = {
        ...
        "google": [
            ("Gemini 2.0 Flash-Lite - Cost efficiency and low latency (RECOMMENDED)", "gemini-2.0-flash-lite"),
